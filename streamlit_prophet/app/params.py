...
from holidays import lockdown_format_func
from mapping import (
...
def input_regressors(
    ...
    all_colls=list(all_cols)
    mask = df[all_colls].isnull().sum() == 0
    eligible_cols = sorted(list(mask[mask].index))
    _print_removed_regressors(sorted(set(all_colls) - set(eligible_cols)))
